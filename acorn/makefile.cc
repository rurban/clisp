...
CC = cc
CFLAGS =  -D__arm -D__riscos -wanp -throwback -JUnix: -IUnix: -DUNICODE -DNO_SIGSEGV -I.
CPP = cc -E
...
makemake : in.makemake config.status
	./config.status --file=makemake

h.unixconf : unixconf.in.h config.status
	sh config.status --header=h.unixconf

Makefile : makemake
	./makemake acorn cc > Makefile.tmp
...
init : h.intparam h.floatparam comment5 varbrace txt2c ccmp2c $(DFILES) h.modules
	$(TOUCH) init
...
varbrace : ^.utils.d.varbrace comment5
	$(COMMENT5) ^.utils.d.varbrace > varbrace.c
	$(CC) $(CFLAGS) $(CLFLAGS) c.varbrace -o varbrace
	$(RM) c.varbrace

txt2c : ^.utils.c.txt2c
	$(CC) $(CFLAGS) $(CLFLAGS)  ^.utils.c.txt2c -o txt2c

ccmp2c : ^.utils.c.ccmp2c
	$(CC) $(CFLAGS) $(CLFLAGS)  -I. ^.utils.c.ccmp2c -o ccmp2c
...
i.charstrg : c.charstrg h.streq h.cjk h.uniwidth c.width h.unitypes h.uniname h.uninames c.lispbibl c.fsubr c.subr c.pseudofun c.constsym c.constobj c.acorn c.uni_upcase c.uni_downcase c.uni_attribute c.uniname h.stdbool h.stdint
	$(CPP) $(CFLAGS) c.charstrg > i.charstrg

s.charstrg : c.charstrg h.streq h.cjk h.uniwidth c.width h.unitypes h.uniname h.uninames c.lispbibl c.fsubr c.subr c.pseudofun c.constsym c.constobj c.acorn c.uni_upcase c.uni_downcase c.uni_attribute c.uniname h.stdbool h.stdint
	$(CC) $(CFLAGS) -S c.charstrg

o.charstrg : c.charstrg h.streq h.cjk h.uniwidth c.width h.unitypes h.uniname h.uninames c.lispbibl c.fsubr c.subr c.pseudofun c.constsym c.constobj c.acorn c.uni_upcase c.uni_downcase c.uni_attribute c.uniname h.stdbool h.stdint
...
RUN= /@.lisp -B . -Efile UTF-8 -Eterminal UTF-8 -norc

fas.modprep : ^.utils.lisp.modprep lisp mem.lispinit
	$(RUN) -M mem.lispinit -q -c ^.utils.lisp.modprep -o fas.modprep

lisp.modprep : ^.utils.lisp.modprep
	-$(RM) lisp.modprep
	-$(LN_S) ^.utils.lisp.modprep lisp.modprep
...
fas.russian : lisp.russian lisp mem.halfcomp
	$(RUN) -m 1000KW -M mem.halfcomp -q -c russian.lisp
...
stage.lisp.russian : lisp.russian
	$(LN_S) lisp.russian stage
...
testsuite-all : suite lisp mem.lispinit
	cd suite && $(MAKE) LEXE= complete
...
h.clisp clisp-c.test : o.genclisph h.unixconf  h.intparam h.floatparam
	...
	(echo; echo '/* genclisph */' ; /@.genclisph clisp-test.c; echo ; echo '#endif /* _CLISP_H */') >> h.clisp
	$(CC) $(CFLAGS) $(CLFLAGS) -DUSE_CLISP_H=1 clisp-c.test -o clisp-test-clisp
	$(CC) $(CFLAGS) $(CLFLAGS) -DUSE_CLISP_H=0 clisp-c.test -o clisp-test-lispbibl
...
linkkit : d.modules c.modules h.clisp lisp.exporting lisp.modprep
	-$(RMRF) linkkit
	mkdir linkkit
	cd linkkit && $(LN_S) ^.d.modules d.modules
	cd linkkit && $(LN_S) ^.c.modules c.modules
	cd linkkit && $(LN_S) ^.h.clisp h.clisp
	cd linkkit && $(LN_S) ^.lisp.exporting lisp.exporting
	cd linkkit && $(LN_S) ^.lisp.modprep lisp.modprep

fas.exporting : ^.modules.lisp.exporting lisp mem.lispinit
	$(RUN) -M mem.lispinit -q -c ^.modules.lisp.exporting -o fas.exporting

lisp.exporting : ^.modules.lisp.exporting
	-$(RM) lisp.exporting
	-$(LN_S) ^.modules.lisp.exporting lisp.exporting
...
clisp-link : in.clisp-link
	sed -e 's%@with_dynamic_modules@%%' -e 's%@createsharedlib@%%' -e 's%@LEXE@%%' < in.clisp-link > clisp-link
	chmod a+x clisp-link

base : modular mem.lispinit
	test -d base || (mkdir base && cd base && for f in lisp.a  h.modules o.modules makevars lisp lispinit.mem; do $(LN_S) ^.$$f $$f; done) || ($(RMRF) base ; exit 1)

anymodule $(MODULES) : lisp mem.lispinit force fas.modprep fas.exporting h.clisp
...
c.clisp : c._clisp c.lispbibl c.fsubr c.subr c.pseudofun c.constsym c.constobj c.acorn h.stdbool h.stdint txt2c
	$(TXT2C) < c._clisp > text.c
	$(CC) $(CFLAGS) $(CLFLAGS) c.text -o text
	/@.text > clisp.c
	$(RM) c.text
	$(RM) text
...
clean0 : force
	-$(RM) core c.varbrace genclisph c.text text
	-$(RMRF) linkkit
...
clean5 : clean4
	...
	-$(RM) lisp.config
	-$(RMRF) data
	-$(RM) clisp clisp.rc comment5 varbrace txt2c ccmp2c
