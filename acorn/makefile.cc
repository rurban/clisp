...
init : h.intparam h.floatparam comment5 ansidecl varbrace txt2c ccmp2c fas.modprep $(DFILES) h.modules
	$(TOUCH) init
...
bench : benchmarks
	CLISP="`pwd`.lisp -M `pwd`.mem.lispinit -B `pwd` -Efile UTF-8 -Eterminal UTF-8 -norc"; export CLISP; cd benchmarks; $(MAKE) CLISP="$$CLISP" clisp

benchmarks :
	-mkdir benchmarks
	cd benchmarks && $(LN_S) ^.^.benchmarks.Makefile .
	cd benchmarks && $(LN_S) ^.^.benchmarks.*.lisp .
...
clean0 : force
	-$(RM) core c.ansidecl c.varbrace genclisph c.text text
	-$(RMRF) linkkit
...
clean5 : clean4
	...
	-$(RM) lisp.config
	-$(RMRF) data
	-$(RM) clisp clisp.rc comment5 ansidecl varbrace txt2c ccmp2c fas.modprep modprep.lib
