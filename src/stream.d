                      ...
                      if (eq(action,S(Kignore)))
                        ...
                        { var chart c = char_code(action);
                          ...
                          if (!(iconv(cd,&inptr1,&insize1,&outptr,&outsize) == (size_t)(-1)))
                            ...
                            { if (errno != EILSEQ) { OS_error(); }
                              else { fehler_unencodable(encoding,*(const chart*)inptr); }
                            }
                        }
                      else
              ...
              { var uintL maximum = 0; # bisheriges Maximum := 0
                while (consp(STACK_0))
                  { var object next = # Line-Position des nächsten Teilstreams
                    ...
                    if (nullp(next)) { skipSTACK(1); return NIL; }
                    if (posfixnum_to_L(next) > maximum) { maximum = posfixnum_to_L(next); } # Maximum nehmen
                    STACK_0 = Cdr(STACK_0);
                  }
...
# < ergebnis: offener File-Stream
  ...
  local object check_open_file_stream(obj)
    var object obj;
    { loop
        { if (!builtin_stream_p(obj)) goto fehler_bad_obj; # Stream ?
