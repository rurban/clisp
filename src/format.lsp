...
(defun format-cs-error (control-string)
  (error-of-type 'type-error
    ...
    (DEUTSCH "~S: Kontrollstring muß ein String sein, nicht ~S"
          ...
          (if colon-modifier
            ...
            (if atsign-modifier
              (let* (; CLtL2 S. 598: "When within a ~{ construct, the "goto" is
                     ...
                     ; iteration." Soll das heißen, daß man bei ~@{ zu Beginn
                     ; jeder Iteration *FORMAT-ARG-LIST* neu binden muß ??
                     ; (*FORMAT-ARG-LIST* *FORMAT-NEXT-ARG*) ??
                     (*FORMAT-CS* inner-cs)
                     (*FORMAT-CSDL* inner-csdl)
                     (*FORMAT-UP-AND-OUT* nil))
...
(flet ((mark-used (blockname)
         ; Markiere den Block, so daß er nicht wegoptimiert wird.
         (setf (get blockname 'used) t)
         ; Markiere alle übersprungenen UNWIND-PROTECTs, so daß sie nicht
         ; wegoptimiert werden.
...
; Diese Form muß nachher mit SUBST ersetzt werden.
                           ...
                           (flet ((compute-inner ()
                                    (if colon-p
                                      ...
                                      (let ((*iterargs* nil))
                                        ...
                                        ; relative to the list of arguments being processed by the
                                        ; iteration." Soll das heißen, daß man bei ~@{ zu Beginn
                                        ; jeder Iteration WHOLE-ARGS neu an ARGS binden muß ??
                                        ; (if atsign-p
