    ...
    #                  falls i>0: schiebe sie um i Bits nach rechts (i=1 geht einfacher).
    {
      if (eq(x,Fixnum_0))
        ...
        if (!R_minusp(y)) {
          ...
          # y<0
          if (I_bignump(y)) {
            goto sign; # y ein Bignum -> Vorzeichen von x zurück
          } else {
            ...
            if (k>=len)
              goto sign; # -y >= intDsize*len -> Vorzeichen von x zurück
    ...
    var object x;
    {
      if (I_fixnump(x)) {
        ...
        if (FN_L_minusp(x,(sint32)x32))
          x32 = ~ x32; # falls <0, komplementieren
  ...
  global uintL I_integer_length(x)
    var object x;
    {
      if (I_fixnump(x)) {
        ...
        if ((sintD)msd < 0)
          msd = ~msd; # falls negativ, invertieren
  ...
  local uintL I_power2p(x)
    var object x;
    {
      if (I_fixnump(x)) {
        ...
        if (!((msd & (msd-1)) == 0))
          return 0; # erstes Digit muss Zweierpotenz sein
        if (test_loop_up(&MSDptr[1],len-1))
          return 0; # danach alles Nullen
        var uintL msdlen;
