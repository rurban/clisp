/* display.c -- changed by Bruno Haible, 23 May 1995 */
...
extern int backspace _PROTO((int count));
...
static char * expand_prompt _PROTO((char *pmt, int *lp));
int rl_expand_prompt _PROTO((char *prompt));
void rl_redisplay _PROTO((void));
static void update_line _PROTO((char *old, char *new, int current_line, int omax, int nmax, int inv_botlin));
int rl_on_new_line _PROTO((void));
int rl_on_new_line_with_prompt _PROTO((void));
int rl_forced_update_display _PROTO((void));
void _rl_move_cursor_relative _PROTO((int new, char *data));
void _rl_move_vert _PROTO((int to));
int rl_show_char _PROTO((int c));
int rl_character_len _PROTO((int c, int pos));
int rl_message ();
int rl_clear_message _PROTO((void));
int rl_reset_line_state _PROTO((void));
void _rl_erase_at_end_of_line _PROTO((int l));
static void clear_to_eol _PROTO((int count));
...
void _rl_update_final _PROTO((void));
static void cr _PROTO((void));
void _rl_redisplay_after_sigwinch _PROTO((void));
...
rl_redisplay ()
{
  ...
  int c_pos = 0, inv_botlin = 0, wrap_offset;
...
rl_on_new_line_with_prompt ()
{
  ...
  for (i = prompt_size; i < line_size; i++)
    { visible_line[i] = 0; invisible_line[i] = 1; }

  _rl_last_c_pos = strlen(rl_prompt);
  _rl_last_v_pos = 0;
  _rl_vis_botlin = last_lmargin = 0;
  return 0;
}
     ...
     int count;
{
  ...
  if (term_IC)
    {
      char *buffer = tgoto (term_IC, 0, count);
     ...
     int count;
{
  ...
  if (term_DC && *term_DC)
    {
      char *buffer = tgoto (term_DC, count, count);
