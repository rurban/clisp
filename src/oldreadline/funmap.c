/* funmap.c -- changed by Bruno Haible, 7 January 1995 */
...
int rl_add_funmap_entry _PROTO((char *name, Function *function));
void rl_initialize_funmap _PROTO((void));
char ** rl_funmap_names _PROTO((void));
static int qsort_string_compare _PROTO((char **s1, char **s2));
     ...
     Function *function;
{
  if (funmap_entry + 2 >= funmap_size)
    if (!funmap)
      ...
      funmap =
	(FUNMAP **)xrealloc ((char *)funmap, (funmap_size += 80) * sizeof (FUNMAP *));
  ...
  for (result_index = 0; funmap[result_index]; result_index++)
      ...
      if (result_index + 2 > result_size)
	{
	  if (!result)
	    ...
	    result = (char **)
	      xrealloc ((char *)result, (result_size += 20) * sizeof (char *));
	}
