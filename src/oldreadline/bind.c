/* bind.c -- changed by Bruno Haible, 16 July 1997 */
...
int rl_add_defun _PROTO((char *name, Function *function, int key));
int rl_bind_key _PROTO((int key, Function *function));
int rl_bind_key_in_map _PROTO((int key, Function *function, Keymap map));
int rl_unbind_key _PROTO((int key));
int rl_unbind_key_in_map _PROTO((int key, Keymap map));
int rl_set_key _PROTO((char *keyseq, Function *function, Keymap map));
int rl_macro_bind _PROTO((char *keyseq, char *macro, Keymap map));
int rl_generic_bind _PROTO((int type, char *keyseq, void *data, Keymap map));
int rl_translate_keyseq _PROTO((char *seq, char *array, int *len));
Function *rl_named_function _PROTO((char *string));
Function *rl_function_of_keyseq _PROTO((char *keyseq, Keymap map, int *type));
int rl_re_read_init_file _PROTO((int count, int ignore));
int rl_read_init_file _PROTO((char *filename));
/* Section "Parser Directives" */
static int parser_if _PROTO((char *args));
static int parser_else _PROTO((char *args));
static int parser_endif _PROTO((char *args));
static int handle_parser_directive _PROTO((char *statement));
int rl_parse_and_bind _PROTO((char *string));
int rl_variable_bind _PROTO((char *name, char *value));
static int glean_key_from_name _PROTO((char *name));
Keymap rl_get_keymap_by_name _PROTO((char *name));
void rl_set_keymap _PROTO((Keymap map));
Keymap rl_get_keymap _PROTO((void));
void rl_set_keymap_from_edit_mode _PROTO((void));
/* Section "Key Binding and Function Information" */
void rl_list_funmap_names _PROTO((int count, int ignore));
char ** rl_invoking_keyseqs_in_map _PROTO((Function *function, Keymap map));
char ** rl_invoking_keyseqs _PROTO((Function *function));
int rl_dump_functions _PROTO((int count, int key));
void rl_function_dumper _PROTO((int print_readably));
void _rl_bind_if_unbound _PROTO((char *keyseq, Function *default_func));
/* Section "String Utility Functions" */
static int substring_member_of_array _PROTO((char *string, char **array));
static char * strindex _PROTO((char *s1, char *s2));
...
#else
/* Section "String Utility Functions" */
#define strnicmp _rl_strnicmp
#define stricmp _rl_stricmp
static int strnicmp _PROTO((char *string1, char *string2, int count));
static int stricmp _PROTO((char *string1, char *string2));
#endif
     ...
     char *string;
{
  ...
  /* Ugly, but working hack to keep prefix-meta around. */
  else if (stricmp (funname, "prefix-meta") == 0)
    {
      ...
      seq[0] = key;
      seq[1] = '\0';
      rl_generic_bind (ISKMAP, seq, emacs_meta_keymap, _rl_keymap);
    }
	...
	case ISFUNC:
	  ...
	  if (map[key].function == function)
		...
		result = (char **) xrealloc
		  ((char *)result, (result_size += 10) * sizeof (char *));
