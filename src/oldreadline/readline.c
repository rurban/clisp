/* readline.c -- changed by Bruno Haible, 16 July 1997 */
...
extern char *tgetstr _PROTO((/* [const] char* id, char** area */));
extern int tgetnum _PROTO((/* [const] char* id */));
extern int tgetflag _PROTO((/* [const] char* id */));
#if !defined(__EMX__)
...
/* Section "Line editing input utility" */
/* Section "Top Level Functions" */
char * readline _PROTO((char *prompt));
static char * readline_internal _PROTO((void));
/* Section "Character Input Buffering" */
int rl_stuff_char _PROTO((int key));
int ibuffer_space _PROTO((void));
int rl_get_char _PROTO((int *key));
int rl_unget_char _PROTO((int key));
void rl_gather_tyi _PROTO((void));
int rl_read_key _PROTO((void));
int _rl_dispatch _PROTO((int key, Keymap map));
/* Section "Hacking Keyboard Macros" */
static void with_macro_input _PROTO((char *string));
static int next_macro_key _PROTO((void));
static void push_executing_macro _PROTO((void));
static void pop_executing_macro  _PROTO((void));
static void add_macro_char _PROTO((int c));
int rl_start_kbd_macro _PROTO((int ignore1, int ignore2));
int rl_end_kbd_macro _PROTO((int count, int ignore));
int rl_call_last_kbd_macro _PROTO((int count, int ignore));
void _rl_kill_kbd_macro _PROTO((void));
/* Section "Initializations" */
int rl_initialize _PROTO((void));
static void readline_initialize_everything _PROTO((void));
static void readline_default_bindings _PROTO((void));
static void bind_arrow_keys_internal _PROTO((void));
static void bind_arrow_keys _PROTO((void));
/* Section "Numeric Arguments" */
static int rl_digit_loop _PROTO((void));
int rl_digit_argument _PROTO((int ignore, int key));
int rl_discard_argument _PROTO((void));
int rl_init_argument _PROTO((void));
int rl_universal_argument _PROTO((void));
/* Section "Terminal and Termcap" */
int rl_reset_terminal _PROTO((char *terminal_name));
void _rl_set_screen_size _PROTO((int tty, int ignore_env));
static void get_term_capabilities _PROTO((char **bp));
static int init_terminal_io _PROTO((char *terminal_name));
char * rl_get_termcap _PROTO((char *cap));
int _rl_output_character_function _PROTO((int c));
void _rl_output_some_chars _PROTO((char *string, int count));
int backspace _PROTO((int count));
int crlf _PROTO((void));
int rl_tty_status _PROTO((int count, int key));
/* Section "Utility Functions" */
int alphabetic _PROTO((int c));
int ding _PROTO((void));
void rl_abort _PROTO((void));
char * rl_copy_text _PROTO((int from, int to));
void rl_extend_line_buffer _PROTO((int len));
/* Section "Insert and Delete" */
int rl_insert_text _PROTO((char *string));
int rl_delete_text _PROTO((int from, int to));
/* Section "Readline character functions" */
/* Section "Movement Commands" */
int rl_forward _PROTO((int count));
int rl_backward _PROTO((int count));
int rl_beg_of_line _PROTO((void));
int rl_end_of_line _PROTO((void));
int rl_forward_word _PROTO((int count));
int rl_backward_word _PROTO((int count));
int rl_refresh_line _PROTO((void));
int rl_clear_screen _PROTO((int count, int key));
int rl_arrow_keys _PROTO((int count, int key));
/* Section "Text commands" */
int rl_insert _PROTO((int count, int c));
int rl_quoted_insert _PROTO((int count, int key));
int rl_tab_insert _PROTO((int count, int key));
int rl_newline _PROTO((int count, int key));
int rl_clean_up_for_exit _PROTO((void));
int rl_do_lowercase_version _PROTO((int ignore1, int ignore2));
int rl_rubout _PROTO((int count));
int rl_delete _PROTO((int count));
int rl_delete_horizontal_space _PROTO((int count, int ignore));
/* Section "Kill commands" */
int rl_unix_word_rubout _PROTO((int count));
int rl_unix_line_discard _PROTO((void));
/* Section "Commands For Typos" */
/* Section "Changing Case" */
int rl_upcase_word _PROTO((int count, int key));
int rl_downcase_word _PROTO((int count, int key));
int rl_capitalize_word _PROTO((int count, int key));
static int rl_change_case _PROTO((int count, int op));
/* Section "Transposition" */
int rl_transpose_words _PROTO((int count, int key));
int rl_transpose_chars _PROTO((int count, int key));
/* Section "Undo, and Undoing" */
void rl_add_undo _PROTO((enum undo_code what, int start, int end, char *text));
void free_undo_list _PROTO((void));
int rl_do_undo _PROTO((void));
int rl_begin_undo_group _PROTO((void));
int rl_end_undo_group _PROTO((void));
int rl_modifying _PROTO((int start, int end));
int rl_revert_line _PROTO((void));
int rl_undo_command _PROTO((int count, int key));
/* Section "History Utilities" */
static void start_using_history _PROTO((void));
void _rl_free_history_entry _PROTO((HIST_ENTRY *entry));
int maybe_replace_line _PROTO((void));
int maybe_unsave_line _PROTO((void));
int maybe_save_line _PROTO((void));
/* Section "History Commands" */
int rl_beginning_of_history _PROTO((int count, int key));
int rl_end_of_history _PROTO((int count, int key));
int rl_get_next_history _PROTO((int count));
int rl_get_previous_history _PROTO((int count));
int rl_execute_next _PROTO((int c));
/* Section "The Mark and the Region." */
int rl_set_mark _PROTO((int position));
int rl_exchange_mark_and_point _PROTO((int count, int key));
/* Section "Killing Mechanism" */
int rl_set_retained_kills _PROTO((int num));
int rl_kill_text _PROTO((int from, int to));
/* Section "Killing Commands" */
int rl_kill_word _PROTO((int count));
int rl_backward_kill_word _PROTO((int count));
int rl_kill_line _PROTO((int direction));
int rl_backward_kill_line _PROTO((int direction));
int rl_kill_full_line _PROTO((int count, int ignore));
int rl_yank _PROTO((void));
int rl_yank_pop _PROTO((int count, int key));
int rl_yank_nth_arg _PROTO((int count, int ignore));
int rl_yank_last_arg _PROTO((int count, int key));
int rl_vi_editing_mode _PROTO((int count, int key));
int rl_emacs_editing_mode _PROTO((int count, int key));
/* Section "USG (System V) Support" */
int rl_getc _PROTO((FILE *stream));
char * savestring _PROTO((char *s));
int uppercase_p _PROTO((int c));
int lowercase_p _PROTO((int c));
int pure_alphabetic _PROTO((int c));
int digit_p _PROTO((int c));
int to_lower _PROTO((int c));
int to_upper _PROTO((int c));
int digit_value _PROTO((int c));
...
readline_internal ()
{
...
#ifdef __GNUC__
  /* force lastc and eof_found to be allocated on the stack,
     otherwise they may be clobbered by the setjmp()/longjmp(). */
  &lastc; &eof_found;
#endif
     ...
     are editing was originally in the history, AND the line has changed. */
  {
	...
	rl_revert_line ();
	entry = replace_history_entry (where_history (), the_line, NULL);
...
readline_initialize_everything ()
{
  ...
  t = getenv ("LC_CTYPE");
  if (t && (strcmp (t, "ISO-8859-1") == 0 || strcmp (t, "iso-8859-1") == 0
	    || strcmp (t, "iso_8859_1") == 0))
    {
      _rl_meta_flag = 1;
      ...
      else
	{
	  ...
	  if (slot == rl_max_kills)
	  ...
	  else
	    {
	      ...
	      rl_kill_ring = (char **)xrealloc ((char *)rl_kill_ring, slot * sizeof (char *));
	    }
     ...
     FILE *stream;
{
  ...
  if (isatty (0))
    { static unsigned char pending = 0;
      ...
      if (pending)
        { ch = pending; pending = 0; return ch; }
      asm __volatile__ ("movb $0x00,%%ah ; .byte 0xcd ; .byte 0x16"
                        : "=a" /* %ax */ (ch)      /* IN */
                        :                          /* OUT */
                        : "bx","cx","dx","si","di" /* %ebx,%ecx,%edx,%esi,%edi */ /* CLOBBER */
                       );
      if ((ch>>8)==0) /* scancode = 0 ? */
        return (ch & 0xFF); /* return Ascii code */
      if ((ch & 0xFF) == 0x00 || (ch & 0xFF) == 0xE0) /* code = 00 or E0 ? */
        { pending = ch>>8; return 0x00; } /* prefix scan code by 0x00 */
      return ch & 0xFF; /* return Ascii code */
    }
#endif /* __EMX_OLD__ || __GO32__ */
