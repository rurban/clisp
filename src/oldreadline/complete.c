/* complete.c -- changed by Bruno Haible, 5 September 1997 */
...
#include <pwd.h>
extern struct passwd *getpwent _PROTO((void));
extern void setpwent _PROTO((void));
extern void endpwent _PROTO((void));
#ifdef __CYGWIN32__
...
int rl_complete _PROTO((int ignore, int invoking_key));
int rl_possible_completions _PROTO((int ignore, int invoking_key));
int rl_insert_completions _PROTO((int ignore, int invoking_key));
int get_y_or_n _PROTO((void));
#if defined (SHELL)
static char * _delete_quotes _PROTO((char *text));
#endif /* SHELL */
static char * printable_part _PROTO((char *pathname));
static int print_filename _PROTO((char *to_print, char *full_pathname));
int rl_complete_internal _PROTO((int what_to_do));
#if defined (VISIBLE_STATS)
static int stat_char _PROTO((char *filename));
#endif /* VISIBLE_STATS */
static int compare_strings _PROTO((char **s1, char **s2));
char * username_completion_function _PROTO((char *text, int state));
/* Section "Completion" */
...
int rl_tilde_expand _PROTO((int ignore, int key));
#if defined (HAVE_STRPBRK)
extern char * strpbrk ();
#define rl_strpbrk strpbrk
#else
     ...
     int what_to_do;
{
  ...
  if (rl_completion_entry_function)
    ...
    our_func = filename_completion_function;
  ...
  if (rl_point)
...
#endif
	{
	  ...
	  if (strchr (rl_basic_quote_characters, scan) && (end - rl_point) > 1)
	    delimiter = scan;
 ...
 after_usual_completion:
  ...
  if (!matches)
      ...
      if (rl_ignore_completion_duplicates)
	{
	  ...
	  /* No! This doesn't work because of signed characters if you
	     are using SunOS 4! */
	  for (i = 0; matches[i]; i++);
	  if (i)
	...
	case '!':
	  ...
	  if (rl_ignore_some_completions_function &&
	      our_func == filename_completion_function)
	    {
	      ...
	      if (matches == (char **)NULL || matches[0] == (char *)NULL)
		{
		  if (matches)
		    free (matches);
		  ding ();
	...
	case '?':
	  ...
	  display_matches:
	    ...
	    if (!rl_ignore_completion_duplicates)
	      qsort (matches+1, len, sizeof (char *), compare_strings);
     ...
     CPFunction *entry_function;
{
  ...
  while ((string = (*entry_function) (text, matches)))
	...
	match_list = (char **)xrealloc
	  ((char *)match_list, ((match_list_size += 10) + 1) * sizeof (char *));
