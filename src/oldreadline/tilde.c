/* tilde.c -- changed by Bruno Haible, 7 January 1995 */
...
static int tilde_find_prefix _PROTO((char *string, int *len));
static int tilde_find_suffix _PROTO((char *string));
char * tilde_expand _PROTO((char *string));
char * tilde_expand_word _PROTO((char *filename));
     ...
     char *filename;
{
  ...
  if (dirname && *dirname == '~')
      ...
      else
	{
	  ...
	  for (i = 1; dirname[i] != '\0' && dirname[i] != '/'; i++)
