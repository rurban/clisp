/* history.c -- changed by Bruno Haible, 5 September 1997 */
...
HISTORY_STATE * history_get_history_state _PROTO((void));
void history_set_history_state _PROTO((HISTORY_STATE *state));
void using_history _PROTO((void));
int history_total_bytes _PROTO((void));
void add_history _PROTO((char *string));
HIST_ENTRY * replace_history_entry _PROTO((int which, char *line, void *data));
int where_history _PROTO((void));
static int history_search_internal _PROTO((char *string, int direction, int anchored));
int history_search _PROTO((char *string, int direction));
int history_search_prefix _PROTO((char *string, int direction));
HIST_ENTRY * remove_history _PROTO((int which));
void stifle_history _PROTO((int max));
int unstifle_history _PROTO((void));
int history_is_stifled _PROTO((void));
static char * history_filename _PROTO((char *filename));
int read_history _PROTO((char *filename));
int read_history_range _PROTO((char *filename, int from, int to));
int history_truncate_file _PROTO((char *fname, int lines));
static int history_do_write _PROTO((char *filename, int nelements, int overwrite));
int append_history _PROTO((int nelements, char *filename));
int write_history _PROTO((char *filename));
HIST_ENTRY * current_history _PROTO((void));
HIST_ENTRY * previous_history _PROTO((void));
HIST_ENTRY * next_history _PROTO((void));
HIST_ENTRY ** history_list _PROTO((void));
HIST_ENTRY * history_get _PROTO((int offset));
int history_search_pos _PROTO((char *string, int dir, int pos));
int history_set_pos _PROTO((int pos));
/* Section "History Expansion" */
char * get_history_event _PROTO((char *string, int *caller_inde, int delimiting_quote));
#if defined (SHELL)
static void rl_string_extract_single_quoted _PROTO((char *string, int *sindex));
static char * quote_breaks _PROTO((char *s));
#endif /* SHELL */
static char * hist_error _PROTO((char *s, int start, int current, int errtype));
static char * get_subst_pattern _PROTO((char *str, int *iptr, int delimiter, int is_rhs, int *lenptr));
static void postproc_subst_rhs _PROTO((void));
static int history_expand_internal _PROTO((char *string, int start, int *end_index_ptr, char **ret_string, char *current_line));
int history_expand _PROTO((char *hstring, char **output));
static char * get_history_word_specifier _PROTO((char *spec, char *from, int *caller_index));
char * history_arg_extract _PROTO((int first, int last, char *string));
static char ** history_tokenize_internal _PROTO((char *string, int wind, int *indp));
char ** history_tokenize _PROTO((char *string));
  ...
  if (history_stifled && (history_length == max_input_history))
      ...
      else
	{
	  if (history_length == (history_size - 1))
		...
		xrealloc ((char *)the_history, history_size * sizeof (HIST_ENTRY *));
	    }
	  history_length++;
	}
    }
     ...
     int from, to;
{
  ...
  fsize = read (file, buffer, finfo.st_size);
  if (fsize < 0)
    {
  error_and_exit:
      if (file >= 0)
  ...
  while (line_start < fsize && current_line < from)
    {
      for (line_end = line_start; line_end < fsize; line_end++)
	if (buffer[line_end] == '\n')
	  {
	    current_line++;
     ...
     char *string;
{
  ...
  if (0 <= first && first < last && last <= len)
    {
     ...
     int wind, *indp;
{
  ...
  while (string[i])
	...
	result = (char **)xrealloc ((char *)result, ((size += 10) * sizeof (char *)));
