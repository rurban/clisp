...
(defmacro defclass (name superclass-specs slot-specs &rest options)
  (unless (symbolp name)
    (error-of-type 'sys::source-program-error
      (DEUTSCH "~S: Klassenname muß ein Symbol sein, nicht ~S"
           ...
           (progn
             ...
             (mapcar #'(lambda (superclass)
                         (unless (symbolp superclass)
                           (error-of-type 'sys::source-program-error
                             (DEUTSCH "~S ~S: Oberklassenname muß ein Symbol sein, nicht ~S"
             ...
             (mapcar #'(lambda (slot-spec)
                         (let ((slot-name slot-spec) (slot-options '()))
                           ...
                           (unless (symbolp slot-name)
                             (error-of-type 'sys::source-program-error
                               (DEUTSCH "~S ~S: Slotname muß ein Symbol sein, nicht ~S"
                                   ...
                                   (:ALLOCATION
                                    ...
                                    (case argument
                                      ...
                                      (t (error-of-type 'sys::source-program-error
                                           (DEUTSCH "~S ~S, Slot-Option zu Slot ~S muß den Wert ~S oder ~S haben, nicht ~S"
...
; In record.d und hier wird benutzt, daß die ersten 4 Attribute übereinstimmen!
...
;             C{i-1} < Ci nicht mehr in R sein, also muß C{i-1} schon in
;             der CPL sein.
;
...
(defun initialize-instance-structure-class
  ...
  (unless (null (cdr direct-superclasses))
    (error-of-type 'error
      (DEUTSCH "~S: Metaklasse STRUCTURE-CLASS läßt nur eine direkte Oberklasse zu."
  ...
  (when direct-slots
    (let* ((more-slots (std-compute-slots class direct-slots))
      ...
      (when (plusp shared-index)
        (error-of-type 'error
          (DEUTSCH "~S: Metaklasse STRUCTURE-CLASS läßt nur keine Shared Slots zu."
...
; Bei CALL-NEXT-METHOD und NO-NEXT-METHOD muß die generische Funktion bekannt
...
; setzt ebenfalls voraus, daß eine generische Funktion ihre Aufrufkonvention
; nicht ändert.
...
; #(C_0 ... C_(k-1) . [Rest von C_k]) zusammengefaßt werden, k = 0 oder 1.
                   ...
                   (if (and (every #'bc-p classes)
                     ...
                     (labels
                        ((built-in-subtree (class remaining-classes remaining-methods)
                           ; behandelt die Fälle, daß das Argument der Klasse
                           ; class angehört und auf Zugehörigkeit zu einer der
                           ; remaining-classes abgeprüft werden muß.
                           ; (Man kann voraussetzen, daß (bc-and class x) /= nil
...
(defun std-add-method (gf method)
  ...
  (let ((gf-sign (gf-signature gf))             ; (reqanz optanz restp keywords allowp)
    ...
    (when (fourth gf-sign) ; gf hat Keywords?
      ; ja -> Methode muß sie akzeptieren:
      (unless (if (fourth m-sign) ; Methode hat &key ?
                (or (sixth m-sign) ; Methode muß &allow-other-keys haben oder
                    (subsetp (fourth gf-sign) (fifth m-sign)) ; die Keywords aufzählen
                )
                (third m-sign) ; Methode muß &rest haben!
              )
        (error-of-type 'error
