...
static void parser_if RL((char* args));
static void parser_else RL((char* args));
static void parser_endif RL((char* args));
static int handle_parser_directive RL((char* statement));
static int glean_key_from_name RL((char* name));
static char ** invoking_keyseqs_in_map RL((VFunction* function, Keymap map));
static int substring_member_of_array RL((char* string, char** array));
#define strnicmp _rl_strnicmp
#define stricmp _rl_stricmp
static int strnicmp RL((char* string1, char* string2, int count));
static int stricmp RL((char* string1, char* string2));
static char *strindex RL((char* s1, char* s2));
...
rl_unbind_key (key)
     int key;
{
  return (rl_bind_key (key, (VFunction *)NULL));
}
...
rl_unbind_key_in_map (key, map)
     ...
     Keymap map;
{
  return (rl_bind_key_in_map (key, (VFunction *)NULL, map));
}
...
rl_set_key (keyseq, function, map)
     ...
     Keymap map;
{
  rl_generic_bind (ISFUNC, keyseq, (void *)function, map);
}
     ...
     Keymap map;
{
  ...
  for (i = 0; i < keys_len; i++)
      ...
      if ((i + 1) < keys_len)
	{
	  if (map[ic].type != ISKMAP)
		...
		free ((char *)map[ic].function);
	      ...
	      map[ic].type = ISKMAP;
	      map[ic].function = (VFunction *)rl_make_bare_keymap ();
	    }
      ...
      else
	{
	  ...
	  map[ic].function = (VFunction *)data;
	  map[ic].type = type;
	}
    }
  free (keys);
     ...
     char *args;
{
  ...
  if (if_stack_depth + 1 >= if_stack_size)
	...
	if_stack = (unsigned char *)xrealloc ((char *)if_stack, if_stack_size += 20);
    }
     ...
     char *string;
{
  ...
  if (*string == '"')
    {
      char *seq = (char *)xmalloc (1 + strlen (string));
      register int j, k = 0;
      int passc = 0;
