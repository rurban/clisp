...
extern struct passwd * getpwent RL((void));
#ifdef __CYGWIN32__
...
#if defined (SHELL)
static char * _delete_quotes RL((char* text));
#endif /* SHELL */
#if defined (VISIBLE_STATS)
static int stat_char RL((char* filename));
#endif /* VISIBLE_STATS */
static int compare_strings RL((char* * s1, char* * s2));
#ifndef HAVE_STRPBRK /* in case the system already has strpbrk(), use it */
static char * strpbrk RL((char* string1, char* string2));
#endif
...
VFunction *rl_ignore_some_completions_function = (VFunction *)NULL;
  ...
  if (rl_filename_completion_desired && rl_visible_stats)
    {
      ...
      extension_char = stat_char (full_pathname);
     ...
     int what_to_do;
{
  ...
  if (rl_point)
      ...
      if (rl_completer_quote_characters)
	{
	  ...
	  for (scan = pass_next = 0; scan < end; scan++)
	      ...
	      if (quote_char != '\0')
		{
	      ...
	      else if (strrchr (rl_completer_quote_characters, rl_line_buffer[scan]))
		{
      ...
      if (strchr (rl_completer_word_break_characters, rl_line_buffer[rl_point]))
	{
	  ...
	  if (strchr (rl_basic_quote_characters, rl_line_buffer[rl_point]) && (end - rl_point) > 1)
  ...
  if (!matches)
	...
	case TAB:
	  ...
	  if (rl_ignore_some_completions_function &&
	      our_func == filename_completion_function)
	    {
	      (*rl_ignore_some_completions_function)(matches);
	  ...
	  if (matches[0] != NULL
	      ...
	      && rl_filename_completion_desired)
	    {
		 ...
		 not be checked, add !matches[1] to the if clause. */
	      if (strpbrk (matches[0], rl_completer_word_break_characters))
	...
	case '?':
	  {
	    ...
	    len = i - 1;
	    /* Consider matches[1] ... matches[len]. */
	    ...
	    /* Watch out for special case.  If LEN is less than LIMIT, then
	       just do the inner printing loop:  0 < len <= limit  implies
               count = 1. */
  ...
  while (directory && (entry = readdir (directory)))
      ...
      else
	{
	  /* Otherwise, if these match upto the length of filename, then
		...
		(entry->d_name[0] == filename[0]) && /* Quick test */
