...
#ifdef HAVE_SYS_STREAM_H /* some SCO systems need this for <sys/ptem.h> */
#include <sys/stream.h>
#endif
#ifdef _SEQUENT_ /* Sequent PTX V4 needs this for <sys/ptem.h> */
#include <sys/termios.h>
#endif
#ifdef HAVE_SYS_PTEM_H /* some SCO systems need this for `struct winsize' */
#include <sys/ptem.h>
#endif
...
extern int tgetflag RL((char* id));
extern char* tgetstr RL((char* id, char** area));
extern char* tgoto RL((char* cm, int destcol, int destline));
extern int tputs RL((char* cp, int affcnt, void (*outcharfun)()));
#endif
...
#include <setjmp.h> /* declares setjmp(), longjmp() */

/* Some standard library routines. */
#include "rltty.h" /* put this first because it may include <sys/termios.h>
		      and thus unconditionally define CTRL */
#include "rlxref.h"
#include "readline.h"
#include "history.h"
...
static void with_macro_input RL((char* string));
static int next_macro_key RL((void));
static void push_executing_macro RL((void));
static void pop_executing_macro RL((void));
static void add_macro_char RL((int c));
/* Section "Changing Case" */
static void rl_change_case RL((int count, int op));
...
VFunction *rl_last_func = (VFunction *)NULL;
     ...
     Keymap map;
{
  ...
  switch (map[key].type)
    ...
    case ISFUNC:
      {
	VFunction *func = map[key].function;
...
static void push_executing_macro RL((void)), pop_executing_macro RL((void));
...
/* Non-zero if we can use the rightmost column of the screen. */
int term_xn;
     ...
     int tty, ignore_env;
{
  ...
  if (!term_xn || _rl_horizontal_scroll_mode)
    screenwidth--;
     ...
     int count;
{
  ...
  if (count > 1 || rl_explicit_arg)
    ...
    {
      int c = (unsigned char) the_line[--rl_point];
      rl_delete_text (rl_point, rl_point + 1);

      if (rl_point == rl_end && (c >= ' ' && c != 127) && _rl_last_c_pos)
	{
	  int l;
