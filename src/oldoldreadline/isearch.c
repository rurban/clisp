/* isearch.c -- changed by Bruno Haible, 17 August 1994 */

/* isearch.c - code for incremental searching in emacs and vi modes. */
...
static void rl_display_search RL((char* search_string, int reverse_p, int where));
static void rl_search_history RL((int direction, int invoking_key));
	...
	default:
	  if (!(c >= ' ' && c < KEYMAP_SIZE && c != 127))
		...
		search_string = (char *)xrealloc
		  ((char *)search_string, (search_string_size += 128));
