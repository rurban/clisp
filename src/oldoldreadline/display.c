/* display.c -- changed by Bruno Haible, 7 January 1995 */
...
static void update_line RL((char* old, char* new, int current_line));
static void clear_to_eol RL((int count));
static void insert_some_chars RL((char* string, int count));
static void delete_chars RL((int count));
...
rl_redisplay ()
{
  ...
  while (line_size <= strlen (prompt_this_line))
    {
      line_size *= 2;
...
/* (PWP) tell the update routines that we have moved onto a
   new line with the prompt already displayed. */
void
rl_on_new_line_with_prompt ()
{
  ...
  /* forced_display > 1 inhibits output of previous prompt lines */
  forced_display *= 2;

  prompt_this_line = strrchr (rl_prompt, '\n');
  if (!prompt_this_line)
    prompt_this_line = rl_prompt;
  else
    prompt_this_line++;

  prompt_size = strlen(prompt_this_line) + 1;
  if (line_size < prompt_size)
    line_size = prompt_size;

  visible_line = (char *)xmalloc(line_size);
  strcpy(visible_line,prompt_this_line);
  invisible_line = (char *)xmalloc(line_size);
  strcpy(invisible_line,prompt_this_line);
  for (i = prompt_size; i < line_size; i++)
    { visible_line[i] = 0; invisible_line[i] = 1; }
  _rl_last_c_pos = strlen(prompt_this_line);
  _rl_last_v_pos = 0;
  _rl_vis_botlin = last_lmargin = 0;
}
     ...
     int count;
{
  ...
  memcpy (row_start + col, row_start + col + count, (width - col - count) * 2);
  ...
  if (term_DC && *term_DC)
    {
      char* buffer = tgoto (term_DC, 0, count);
      tputs (buffer, 1, _rl_output_character_function);
    }
  else
    {
