...
#define member(c, s) ((c) ? strchr ((s), (c)) : 0)

static char error_pointer;

/* Forward declarations. */
static int history_search_internal RL((char* string, int direction, int anchored));
static char *history_filename RL((char* filename));
static int history_do_write RL((char* filename, int nelements, int overwrite));
static char *get_history_word_specifier RL((char* spec, char* from, int* caller_index));
static char * history_find_word RL((char *line, int ind));
     ...
     int delimiting_quote;
{
  ...
  if (digit (string[i]))
    ...
    get_which:
      if ((entry = history_get (which)) != NULL)
	return (entry->line);
 ...
 grovel:
  ...
  for (i = j = 0; i < l; i++)
	...
	do_add:
	  j += strlen (temp);
	  while (j > (len - 1))
	    result = (char *)xrealloc ((char *)result, (len += 256));
  ...
  if (spec[i] == '%')
    {
      *caller_index = i + 1;
      if (search_match)
	return (savestring (search_match));
      else
	return (savestring (""));
    }
 ...
 get_token:
  ...
  /* Get word from string + i; */
  {
	...
	if (!size)
	  ...
	  result =
	    (char **)xrealloc ((char *)result, ((size += 10) * (sizeof (char *))));
      }
