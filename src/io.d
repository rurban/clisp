    ...
    switch(scode) {
      ...
      case syntax_eof:              /* EOF reached. */
        if (multiple_escape_flag) { /* between multiple-escape-character? */
          ...
          pushSTACK(*stream_);
          pushSTACK(S(readL));
          error(end_of_file,GETTEXT("~S: input stream ~S ends within a token after multiple escape character"));
        }
...
/* ----------- READ between token-level and object-level -------------- */
  ...
  *base_ = 10;
  { /* 5. Floating-Point-Number or decimal-integer */
    ...
    /* flags & bit(2)  indicates, if an a_dot has arrived already
       ...
       flags & bit(3)  indicates whether in the last digit block already
                       a_digit was found
       flags & bit(4)  indicates, if there was an a_dot with digits in front
                       of it */
    while (index<len) { /* next character */
      var uintB attr = *attrptr++; /* its attribute code */
      if (attr==a_digit) {
        /* Digit ('0'-'9') */
        flags |= bit(3); index++;
      } else if (attr==a_dot) {
...
local maygc Values read_macro (object ch, const gcv_object_t* stream_) {
  ...
  if (!simple_vector_p(macrodef)) { /* a simple-vector? */
    ...
    if (mv_count > 1) {
      ...
      pushSTACK(*stream_);
      pushSTACK(S(readL));
      error(error_condition,GETTEXT("~S from ~S: macro character definition for ~S may not return ~S values, only one value."));
    }
    /* at most one value. */
    ...
    if (nullp(macrodef)) {                       /* NIL -> undefined */
      ...
      pushSTACK(*stream_);      /* Stream */
      pushSTACK(S(readL));
      error(reader_error, /* ANSI CL spec of MAKE-DISPATCH-MACRO-CHARACTER wants a reader-error here */
    ...
    if (mv_count > 1) {
      ...
      pushSTACK(*stream_);         /* Stream */
      pushSTACK(S(readL));
      error(error_condition,GETTEXT("~S from ~S: dispatch macro character definition for ~S after ~S may not return ~S values, only one value."));
    }
    /* at most 1 value. */
    return;                     /* retain mv_space/mv_count */
  }
 ...
 wloop: { /* loop for skipping of leading whitespace/comment: */
  ...
  if (!token_escape_flag && test_dots()) {
    ...
    if (len > 1) {              /* Length>1 -> error */
      ...
      pushSTACK(*stream_);
      pushSTACK(S(readL));
      error(reader_error, /* ANSI CL 2.4.9. wants a reader-error here */
...
/* UP: reads an Object, with SYS::*READ-RECURSIVE-P* /= NIL
 ...
 > stream: Stream
 < stream: Stream
 < result: read Object (dot_value at single dot)
 can trigger GC */
...
local _Noreturn void error_dot (object stream) {
  ...
  pushSTACK(S(readL));
  error(reader_error,    /* ANSI CL 2.3.3. wants a reader-error here. */
...
local object check_read_reference_table (void) {
  ...
  pushSTACK(S(readL)); pushSTACK(S(read_reference_table));
  pushSTACK(TheSubr(subr_self)->name);
  error(error_condition,GETTEXT("~S: symbol ~S is not bound, it appears that top-level ~S was called with a non-NIL recursive-p argument"));
}
...
local object make_references (object obj) {
  ...
  if (nullp(alist)) {
    ...
    if (!eq(bad_reference,nullobj)) {
      ...
      pushSTACK(S(readL));
      error(reader_error,GETTEXT("~S: no entry for ~S from ~S in ~S = ~S"));
    }
    return popSTACK();
  }
}
...
/* UP: reads an Object.
 ...
 < result: read Object (eof_value at EOF, dot_value at single dot)
 can trigger GC */
global maygc object stream_read (const gcv_object_t* stream_,
                                 object recursive_p, object whitespace_p) {
...
/* UP: Read List.
 ...
 < result: read Object
 can trigger GC */
local maygc object read_delimited_list (const gcv_object_t* stream_,
                                        object endch, object ifdotted);
/* Dito with set SYS::*READ-RECURSIVE-P* : */
local maygc object read_delimited_list_recursive
(const gcv_object_t* stream_, object endch, object ifdotted);
    ...
    error_dot(*stream_);
  {
    ...
    while (1) {           /* loop, in order to read last List-element */
      ...
      error_dot:
        ...
        error(reader_error,GETTEXT("~S from ~S: illegal end of dotted list"));
      }
      if (scode < syntax_t_macro) { /* Macro-Character? */
        /* no -> read last Object: */
        object1 = read_recursive_no_dot(stream_);
...
LISPFUNN(rpar_reader,2) {       /* reads ) */
  ...
  pushSTACK(*stream_);          /* stream */
  pushSTACK(S(readL));
  error(reader_error,GETTEXT("~S from ~S: an object cannot start with ~S"));
}
   ...
   (let ((token (read-token stream)))
     (unless *read-suppress*
       ...
       (if (case (test-number-syntax token base)
         ...
         (error "~S of ~S: The token ~S after #~C cannot be interpreted as rational number in base ~S."
   ...
   #'(lambda (stream sub-char n)
       (if *read-suppress*
         (if (and n (<= 2 n 36))
           ...
           (error "~S from ~S: The base ~S given between #"" and R should lie between 2 and 36." 'read stream))
...
LISPFUNN(radix_reader,3) {                   /* reads #R */
  ...
  if (nullp(STACK_0)) {
    ...
    pushSTACK(*stream_);        /* Stream */
    pushSTACK(S(readL));
    error(reader_error,GETTEXT("~S from ~S: the number base must be given between #"" and R"));
  }
  var uintV base;
...
LISPFUNN(complex_reader,3) {                       /* reads #C */
  ...
  if (!consp(obj)) goto bad;    /* obj must be a Cons ! */
  {
    ...
    if_realp(Car(obj), ; , goto bad; ); /* and (car obj) being a real! */
    if_realp(Car(obj2), ; , goto bad; ); /* and (car obj2) being a real! */
...
local uintV read_vector_length_check (uintV token_length,object type,
  ...
  if (nullp(STACK_0)) {
    ...
    if (n >= vbit(intLsize)) {
      ...
      pushSTACK(*stream_);             /* Stream */
      pushSTACK(S(readL));
      error(type_error,GETTEXT("~S from ~S: invalid ~S length ~S"));
    }
   #endif
    return n;
  }
  ...
  buff_1 = TLO(token_buff_1);
  {
    ...
    for (; index < n; index++) {
      ...
      if (chareq(ch,ascii('0'))) {
        ...
        sbvector_bset(bv,index); /* One -> set Bit */
      }
    }
  }
       ...
       (if *read-suppress*
         ...
         (if (null n)
           ...
           (let* ((rank n)
             ...
             (make-array (nreverse dims) :element-type eltype
...
LISPFUNN(array_reader,3) {                             /* reads #A */
  ...
  if (nullp(STACK_0)) {       /* n not specified? */
   ...
   bad:
    ...
    pushSTACK(*stream_);        /* Stream */
    pushSTACK(S(readL));
    error(reader_error,GETTEXT("~S from ~S: bad syntax for array: #A~S"));
  }
...
local _Noreturn void error_read_eval_forbidden (const gcv_object_t* stream_, object obj) {
  ...
  pushSTACK(*stream_);          /* Stream */
  pushSTACK(S(readL));
  error(reader_error,   /* ANSI CL 2.4.8.6. wants a reader-error here */
   ...
   #'(lambda (stream sub-char n)
       (unless *read-suppress*
         (if n
           ...
           (error "~S of ~S: Between #and #a number must be specified."
                  'read stream)))))
...
LISPFUNN(label_definition_reader,3) { /* reads #= */
  ...
  if (consp(lookup)) {
    ...
    if (eq(obj,STACK_1)) {      /* read object = label ? */
      ...
      pushSTACK(*stream_);      /* Stream */
      pushSTACK(S(readL));
      error(reader_error,GETTEXT("~S from ~S: #~S= #~S#"" is illegal"));
    }
...
LISPFUNN(label_reference_reader,3) { /* reads ##*/
  ...
  if (consp(lookup)) {
    ...
    pushSTACK(STACK_(2+2));     /* Stream */
    pushSTACK(S(readL));
    error(reader_error,GETTEXT("~S from ~S: undefined label #~S#"));
  }
}
...
LISPFUNN(not_readable_reader,3) { /* reads #< */
  ...
  pushSTACK(*stream_);          /* Stream */
  pushSTACK(S(readL));
  error(reader_error,  /* ANSI CL 2.4.8.20. wants a reader-error here */
...
local maygc Values feature (uintWL demandvalue) {
  ...
  if (interpret_feature(expr) == demandvalue) { /* truth value "true" */
    /* read next Object and set for value: */
...
LISPFUNN(structure_reader,3) {                 /* reads #S */
  ...
  if (atomp(args)) {
    ...
    pushSTACK(*stream_);        /* Stream */
    pushSTACK(S(readL));
    error(reader_error,GETTEXT("~S from ~S: #S must be followed by the type and the contents of the structure, not ~S"));
  }
  {
    ...
    if (!symbolp(name)) {       /* Type must be a Symbol ! */
      ...
      pushSTACK(*stream_);      /* Stream */
      pushSTACK(S(readL));
      error(reader_error,GETTEXT("~S from ~S: the type of a structure should be a symbol, not ~S"));
    }
    pushSTACK(name);
    ...
    if (eq(name,S(hash_table))) { /* Symbol HASH-TABLE ? */
      ...
      if (!consp(args)) {
        pushSTACK(*stream_);    /* STREAM-ERROR slot STREAM */
        pushSTACK(S(hash_table)); pushSTACK(*stream_); pushSTACK(S(readL));
    ...
    if (eq(name,S(function))) { /* Symbol FUNCTION */
      ...
      var gcv_object_t* data = TheSrecord(closure)->recdata;
      var uintL pos = 0;
      args = STACK_1;           /* restore */
      ...
      { /* execute (GET name 'SYS::DEFSTRUCT-DESCRIPTION): */
        ...
        if (boundp(description)) {
          ...
          if (simple_vector_p(description)
            ...
            pushSTACK(*stream_); /* Stream */
            pushSTACK(S(readL));
            error(reader_error,GETTEXT("~S from ~S: bad ~S for ~S"));
          }
        }
      }
  ...
  while (1) {                 /* process remaining Argumentlist, */
    ...
    *(stream_ STACKop -1) = Cdr(args); /* shorten Arglist */
    ...
    if (argcount == 0) {
      ...
      pushSTACK(*stream_);              /* Stream */
      pushSTACK(S(readL));
      error(reader_error,GETTEXT("~S from ~S: too many slots for structure ~S"));
    }
  }
...
local _Noreturn void error_closure_badchar (void) {
  ...
  pushSTACK(STACK_(2+2));       /* Stream */
  pushSTACK(S(readL));
  error(reader_error,GETTEXT("~S from ~S: illegal syntax of closure code vector after #~SY"));
}
  ...
  if (nullp(STACK_0)) {
   ...
   #undef OPTARG
    ...
    if (n >= vbit(intLsize)) {
      ...
      pushSTACK(*stream_);      /* Stream */
      pushSTACK(S(readL));
      error(type_error,GETTEXT("~S from ~S: invalid code vector length ~S"));
    }
   #endif
...
LISPFUNN(ansi_pathname_reader,3) {                 /* reads #P */
  ...
  if (stringp(obj)) {
    ...
    pushSTACK(*stream_);        /* Stream */
    pushSTACK(S(readL));
    error(reader_error,GETTEXT("~S from ~S: bad syntax for pathname: #P~S"));
  }
}
...
local Values eof_handling (int mvc) {
  if (!nullp(STACK_2)) {  /* eof-error-p /= NIL (e.g. = #<UNBOUND>) ? */
    ...
    if (missingp(recursive_p))
      ...
      error_eof_inside(&STACK_3); /* report EOF within Object */
  } else {                        /* handle EOF: */
...
LISPFUN(read_from_string,seclass_default,1,2,norest,key,3,
  ...
  value2 = value1; value1 = popSTACK(); /* Index as 2nd, Object as 1st value */
  mv_count=2;
}
...
LISPFUN(parse_integer,seclass_rd_sig,1,0,norest,key,4,
 ...
 badsyntax: /* illegal character */
  if (!junk_allowed) { /* signal an error */
    ...
    pushSTACK(TheSubr(subr_self)->name);
    error(parse_error,GETTEXT("~S: substring ~S does not have integer syntax at position ~S"));
  }
...
/* UP: prints simple-string in opposite case
 ...
 < stream: Stream
 can trigger GC */
local maygc void write_sstring_invert(const gcv_object_t* stream_,
                                      object string) {
  sstring_un_realloc(string);
...
/* UP: prints simple-string according to case_sensitive, case_inverted,
 ...
 < stream: Stream
 can trigger GC */
local maygc void write_sstring_case_ext (const gcv_object_t* stream_,
                                         object string, bool case_sensitive,
                                         bool case_inverted) {
  if (case_sensitive) {
    if (case_inverted)
    ...
    switch (RTCase(readtable)) {
      ...
      case case_preserve:
        ...
        if (case_inverted)
          ...
          write_sstring(stream_,string);
        break;
      case case_invert: {       /* ignore *PRINT-CASE*. */
        var bool seen_uppercase = false;
        var bool seen_lowercase = false;
        var uintL count = Sstring_length(string);
        if (count > 0) {
...
local object right_margin (void) {
  ...
  if (nullp(prm))
    ...
    error(error_condition,GETTEXT("~S: must be a positive integer or NIL, not ~S"));
  }
...
local maygc void pr_enter_1 (const gcv_object_t* stream_, object obj,
  ...
  if (!nullpSv(print_pretty)) {
    ...
    if (!PPHELP_STREAM_P(*stream_)) {    /* still a normal stream */
      ...
      skip_NL: { /* print first element, if string */
          var object top = Car(STACK_0);
          if (stringp(top)) {
...
/* The same routine with treatment of *PRINT-CIRCLE*, *PRINT-PRETTY*
 and SYS::*PRIN-STREAM* : */
local void pr_enter (const gcv_object_t* stream_, object obj,
                     pr_routine_t* pr_xxx) {
...
local pr_routine_t* special_list_p (object obj, bool dotted_p) {
  ...
  if (!dotted_p && eq(head,S(quote))) { /* QUOTE */
    ...
    return (pr_routine_t*)NULL;
 test2bq: {
    /* test, if SYS::*PRIN-BQLEVEL* > 0 and if obj is a list of length 2. */
...
local maygc void pr_list_bothsplice (const gcv_object_t* stream_,
                                     object list, object ch) {
...
typedef maygc void pr_array_elt_routine_t (const gcv_object_t* stream_,
                                           object obj, pr_array_info_t* info);
...
local maygc void pr_array_recursion (pr_array_locals_t* locals, uintL depth,
                                     uintL rdepth) {
...
/* -------- Structures --------
 ...
 can trigger GC */
local maygc void pr_structure_external (const gcv_object_t* stream_,
...
/* UP: print structure to stream.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_structure_default (const gcv_object_t* stream_,
                                       object structure) {
  var object name = Car(TheStructure(structure)->structure_types);
  /* name = (car '(name_1 ... name_i-1 name_i)) = name_1. */
  pushSTACK(structure);
  ...
  if (boundp(clas)) { /* print structure with slot-name: */
    ...
    if (matomp(TheClass(clas)->current_version)) {
     bad_clas:
      pushSTACK(S(print));
      error(error_condition,GETTEXT("~S: bad class"));
    }
    ...
    { /* loop through slot-list STACK_0 = (clos:class-slots clas) : */
      ...
      while (mconsp(STACK_0)) {
        ...
        pushSTACK(*(structure_ STACKop -1)); /* name as 1st Argument */
        pushSTACK(*(structure_ STACKop 0)); /* Structure as 2nd Argument */
...
/* UP: prints a list as the rest of a record.
 ...
 < stream: stream
 can trigger GC */
local maygc void pr_record_rest (const gcv_object_t* stream_, object obj,
                                 uintL length) {
  var uintL length_limit = get_print_length(); /* *PRINT-LENGTH*-limit */
  pushSTACK(obj);
  while (mconsp(STACK_0)) {
...
LISPFUN(pprint_indent,seclass_default,2,1,norest,nokey,0,NIL) {
  ...
  var sintV offset = 0;
  {
    ...
    if (!fixnump(num)) {
     bad_num:
      ...
      error(type_error,GETTEXT("~S: argument ~S is too large"));
    }
  ...
  if (eq(S(Kblock),STACK_2)) {
    ...
    error(type_error,GETTEXT("~S: argument ~S should be ~S or ~S."));
  }
