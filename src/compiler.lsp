...
CODEVEC = Code-Vektor, ein SIMPLE-BIT-VECTOR,
                 2 Bytes : maximale SP-Tiefe, 1-Anteil
                 2 Bytes : maximale SP-Tiefe, jmpbufsize-Anteil
...
(LOADI k1 k2 n)               A0 := ((SP+4*k)+4*n), 1 Wert

(LOADC n m)                   A0 := (svref (STACK+4*n) 1+m), 1 Wert

(LOADV k m)                   A0 := (svref ... m)
                                    (svref ... 0) ; k mal wiederholt
                                    VenvConst,
                              1 Wert

(LOADIC k1 k2 n m)            A0 := (svref ((SP+4*k)+4*n) 1+m), 1 Wert

(STORE n)                     (STACK+4*n) := A0, 1 Wert

(STOREI k1 k2 n)              ((SP+4*k)+4*n) := A0, 1 Wert
...
(STOREIC k1 k2 n m)           (svref ((SP+4*k)+4*n) 1+m) := A0, 1 Wert
...
(SKIPI k1 k2 n)               STACK := (SP+4*k)+4*n, SP:=SP+4*(k+1)

(SKIPSP k1 k2)                SP := SP+4*k
...
(SKIP&RETGF n)                STACK := STACK+4*n, beendet ausnahmsweise die
                              Funktion mit 1 Wert A0/..., normalerweise
                              aber wird A0 als Funktion aufgerufen, mit den-
                              selben Argumenten wie die aktuelle Funktion
...
(JMPHASHV n label)            Sucht A0 in der EQ- oder EQL-Hash-Tabelle
                              (svref (CONST 0) n). Gefunden: Sprung ans von
                              GETHASH gelieferte Label. Nicht gefunden: Sprung
                              zu label. Undefinierte Werte.
...
(RETURN-FROM-I k1 k2 n)       Verlasse den Block, dessen Block-Cons
                              ((SP+4*k)+4*n) ist, mit den Werten A0/...
...
(TAGBODY-OPEN n label1 ... labelm)
                              Legt einen Tagbody-Cons (mit CAR = (CONST n),
                              einem Simple-Vector der Länge m, und
                              CDR=Framepointer) auf -(STACK) ab, baut einen
                              Tagbody-Frame auf. Bei einem GO mit Nummer l
                              wird zu labell gesprungen. Undefinierte Werte.
...
(GO n l)                      Springe im Tagbody, dessen Tagbody-Cons
                              (CONST n) ist, an Tag Nummer l

(GO-I k1 k2 n l)              Springe im Tagbody, dessen Tagbody-Cons
                              ((SP+4*k)+4*n) ist, an Tag Nummer l
...
(14) Instruktionen für HANDLER-BIND

Mnemonic                      Bedeutung

(HANDLER-OPEN n)              baut einen HANDLER-Frame auf; (CONST n) enthält
                              die Condition-Typen, die entsprechenden Labels
                              und die aktuelle SP-Tiefe

(HANDLER-BEGIN&PUSH)          beginnt einen Handler: stellt den SP-Zustand
                              wie beim HANDLER-OPEN her,
                              A0 := dem Handler übergebene Condition, 1 Wert,
                              -(STACK) := A0


(15) Kurz-Instruktionen für einige Funktionen
...
(LIST* n)                     Bildet eine Liste aus den untersten n auf dem STACK
                              liegenden Objekten und A0, STACK := STACK + 4*n,
                              Liste nach A0, 1 Wert
...
(defconstant c-typep-alist1
  '((ARRAY . arrayp)
    ...
    (FLOAT . floatp)
    (FUNCTION . functionp)
    (CLOS:GENERIC-FUNCTION . clos::generic-function-p)
    (HASH-TABLE . hash-table-p)
    (INTEGER . integerp)
    (KEYWORD . keywordp)
    (LIST . listp)
    #+LOGICAL-PATHNAMES
    (LOGICAL-PATHNAME . sys::logical-pathname-p)
    ...
    (CLOS:STANDARD-OBJECT . clos::std-instance-p)
    (STREAM . streamp)
    (FILE-STREAM . sys::file-stream-p)
    (SYNONYM-STREAM . sys::synonym-stream-p)
    (BROADCAST-STREAM . sys::broadcast-stream-p)
    (CONCATENATED-STREAM . sys::concatenated-stream-p)
    (TWO-WAY-STREAM . sys::two-way-stream-p)
    (ECHO-STREAM . sys::echo-stream-p)
    (STRING-STREAM . sys::string-stream-p)
    (STRING . stringp)
    (CLOS:STRUCTURE-OBJECT . clos::structure-object-p)
    (SYMBOL . symbolp)
    (VECTOR . vectorp)
)  )
...
(defconstant c-typep-alist3
  `((ARRAY . ,(c-typep-array 'ARRAYP))
    ...
    (CONS .
      ,#'(lambda (x &optional (car-type '*) (cdr-type '*) &rest illegal-args)
           (declare (ignore illegal-args))
           `(AND (CONSP ,x)
