      ...
      (progn
        (when (get symbol 'SYSTEM::SETF-EXPANDER)
          (warn (DEUTSCH "Die Funktion (~S ~S) ist durch einen SETF-Expander verborgen."
                 ...
                 FRANCAIS "La fonction (~S ~S) est cachå∏e par une må∏thode SETF.")
                'setf symbol
        ) )
...
(defun get-setf-expansion (form &optional (env (vector nil nil)))
                            ...
                            (if (keywordp argform)
                              ...
                              (error-of-type 'source-program-error
                                (DEUTSCH "Das Argument ~S zu ~S sollte ein Keyword sein."
                                 ...
                                 FRANCAIS "L'argument ~S de ~S doit åπtre un mot-clå∏.")
  ...
  (cond ((symbolp form)
        ...
        (t (error-of-type 'source-program-error
             (DEUTSCH "Das Argument muåÍ eine 'SETF-place' sein, ist aber keine: ~S"
              ENGLISH "Argument ~S is not a SETF place."
              FRANCAIS "L'argument ~S doit reprå∏senter une place modifiable.")
             form
  )     )  )
)
    ...
    (unless (and (consp stores) (null (cdr stores)))
      (error-of-type 'source-program-error
        (DEUTSCH "Diese 'SETF-place' produziert mehrere 'Store-Variable': ~S"
         ...
         FRANCAIS "La place modifiable ~S produit plusieurs variables de rå∏sultat.")
        form
    ) )
...
(defun documentation (symbol doctype)
  (unless (function-name-p symbol)
    (error-of-type 'error
      (DEUTSCH "~S: Das ist als erstes Argument unzulå¥ssig, da kein Symbol: ~S"
...
(defmacro define-setf-expander (accessfn lambdalist &body body
  ...
  (unless (symbolp accessfn)
    (error-of-type 'source-program-error
      (DEUTSCH "Der Name der Access-Function muåÍ ein Symbol sein und nicht ~S."
       ENGLISH "The name of the access function must be a symbol, not ~S"
       FRANCAIS "Le nom de la fonction d'accå≈s doit åπtre un symbole et non ~S.")
      accessfn
  ) )
              ...
              (setq mainform
                `(IF ,lengthtest
                   (ERROR-OF-TYPE 'PROGRAM-ERROR
                     (DEUTSCH "Der SETF-Expander fåÕr ~S kann nicht mit ~S Argumenten aufgerufen werden."
                      ENGLISH "The SETF expander for ~S may not be called with ~S arguments."
                      FRANCAIS "L'Å´expandeurÅª SETF pour ~S ne peut pas åπtre appelå∏ avec ~S arguments.")
  ...
  (cond ((and (consp args) (not (listp (first args))) (symbolp (first args)))
            ...
            (LET ()
              ...
              (SYSTEM::%SET-DOCUMENTATION ',accessfn 'SETF
                ,(if (and (null (cddr args))
                   ...
                   (if (cddr args)
                     (error-of-type 'source-program-error
                       (DEUTSCH "Zu viele Argumente fåÕr DEFSETF: ~S"
                        ENGLISH "Too many arguments to DEFSETF: ~S"
                        FRANCAIS "Trop d'arguments pour DEFSETF : ~S")
                       (cdr args)
                     )
                     (error-of-type 'source-program-error
                       (DEUTSCH "Der Dok.-String zu DEFSETF muåÍ ein String sein: ~S"
                        ENGLISH "The doc string to DEFSETF must be a string: ~S"
                        FRANCAIS "La documentation pour DEFSETF doit åπtre un chaåΩne : ~S")
                       (second args)
                 ) ) )
              )
              ',accessfn
        ...
        ((and (consp args) (listp (first args)) (consp (cdr args)) (listp (second args)))
         (when (null (second args))
                (error-of-type 'source-program-error
              (DEUTSCH "Bei DEFSETF muåÍ genau eine 'Store-Variable' angegeben werden."
                   ENGLISH "Missing store variable in DEFSETF."
               FRANCAIS "Une variable de rå∏sultat doit åπtre prå∏ciså∏e dans DEFSETF.")))
         (multiple-value-bind (body-rest declarations docstring)
        ...
        (t (error-of-type 'source-program-error
             (DEUTSCH "DEFSETF-Aufruf fåÕr ~S ist falsch aufgebaut."
              ENGLISH "Illegal syntax in DEFSETF for ~S"
              FRANCAIS "Le DEFSETF ~S est mal formå∏.")
             accessfn
) )     )  )
;;;----------------------------------------------------------------------------
...
(defun SYSTEM::%SETNTH (index list value)
  (let ((pointer (nthcdr index list)))
    (if (null pointer)
      (error-of-type 'error
        (DEUTSCH "(SETF (NTH ...) ...) : Index ~S ist zu groåÍ fåÕr ~S."
               ...
               (when (atom (cdr args))
                 (error-of-type 'source-program-error
                    (DEUTSCH "PSETF mit einer ungeraden Anzahl von Argumenten aufgerufen: ~S"
                     ...
                     FRANCAIS "PSETF fut appelå∏ avec un nombre impair d'arguments : ~S")
                    form))
    ...
    (let* ((indicatorvar (gensym))
          ...
          `(LET* ,(nreverse bindlist)
             (DO ((,var1 ,(first SM3) (CDDR ,var1))
               ...
               (COND ((ATOM (CDR ,var1))
                      (ERROR-OF-TYPE 'ERROR
                   (DEUTSCH "REMF: Property-Liste ungerader Lå¥nge aufgetreten."
                         ENGLISH "REMF: property list with an odd length"
                    FRANCAIS "REMF : Occurence d'une liste de propriå∏tå∏s de longueur impaire.")))
    ...
    (do* ((lambdalistr lambdalist (cdr lambdalistr))
      ...
      (cond ((eq next '&OPTIONAL))
             ...
             (if (symbolp (second lambdalistr))
               ...
               (error-of-type 'source-program-error
                 (DEUTSCH "In der Definition von ~S ist die &REST-Variable kein Symbol: ~S"
                  ...
                  FRANCAIS "Dans la då∏finition de ~S la variable pour &REST n'est pas un symbole : ~S.")
            ...
            ((or (eq next '&KEY) (eq next '&ALLOW-OTHER-KEYS) (eq next '&AUX))
             (error-of-type 'source-program-error
               (DEUTSCH "In einer DEFINE-MODIFY-MACRO-Lambdaliste ist ~S unzulå¥ssig."
            ...
            (t (error-of-type 'source-program-error
                 (DEUTSCH "Lambdalisten dåÕrfen nur Symbole und Listen enthalten, nicht aber ~S"
          ...
          ((oddp argcount)
           (error-of-type 'source-program-error
             (DEUTSCH "~S mit einer ungeraden Anzahl von Argumenten aufgerufen: ~S"
              ...
              FRANCAIS "~S fut appelå∏ avec un nombre impair d'arguments : ~S")
             'setf form
          ))
  ...
  (when (< (length args) 2)
    (error-of-type 'source-program-error
      (DEUTSCH "SHIFTF mit zu wenig Argumenten aufgerufen: ~S"
       ...
       FRANCAIS "SHIFTF fut appelå∏ avec trop peu d'arguments : ~S")
      form))
  (do* ((arglist args (cdr arglist))
...
(defun sys::%putf (plist indicator value)
  (do ((plistr plist (cddr plistr)))
    ...
    (when (atom (cdr plistr))
      (error-of-type 'error
        (DEUTSCH "(SETF (GETF ...) ...) : Property-Liste ungerader Lå¥nge aufgetaucht."
         ENGLISH "(SETF (GETF ...) ...) : property list with an odd length"
         FRANCAIS "(SETF (GETF ...) ...) : Occurence d'une liste de propriå∏tå∏s de longueur impaire.")
    ))
...
(defun SYSTEM::%SET-DOCUMENTATION (symbol doctype value)
  (unless (function-name-p symbol)
    (error-of-type 'error
      (DEUTSCH "Das ist als erstes Argument unzulå¥ssig, da kein Symbol: ~S"
...
(define-setf-expander APPLY (fun &rest args &environment env)
  (if (and (listp fun)
    ...
    (setq fun (second fun))
    (error-of-type 'source-program-error
      (DEUTSCH "SETF von APPLY ist nur fåÕr Funktionen der Form #'symbol als Argument definiert."
       ENGLISH "SETF APPLY is only defined for functions of the form #'symbol."
       FRANCAIS "Un SETF de APPLY n'est då∏fini que pour les fonctions de la forme #'symbole.")
  ) )
  (multiple-value-bind (SM1 SM2 SM3 SM4 SM5)
      (get-setf-expansion (cons fun args) env)
    (unless (eq (car (last args)) (car (last SM2)))
      (error-of-type 'source-program-error
        (DEUTSCH "APPLY von ~S kann nicht als 'SETF-Place' aufgefaåÍt werden."
         ENGLISH "APPLY on ~S is not a SETF place."
         FRANCAIS "APPLY de ~S ne peux pas åπtre considå∏rå∏ comme une place modifiable.")
        fun
    ) )
    (let ((item (car (last SM1)))) ; 'item' steht fåÕr eine Argumentliste!
      (labels ((splice (arglist)
                 ; WåÕrde man in (LIST . arglist) das 'item' nicht als 1 Element,
                 ; sondern gespliced, sozusagen als ',@item', haben wollen, so
                 ; brå¥uchte man die Form, die (splice arglist) liefert.
                 (if (endp arglist)
                   'NIL
                   (let ((rest (splice (cdr arglist))))
                     (if (eql (car arglist) item)
                       ; ein (APPEND item ...) davorhå¥ngen, wie bei Backquote
                       (backquote-append item rest)
                       ; ein (CONS (car arglist) ...) davorhå¥ngen, wie bei Backquote
...
;;;----------------------------------------------------------------------------
;;; Zuså¥tzliche Definitionen von places
;;;----------------------------------------------------------------------------
(define-setf-expander funcall (fun &rest args &environment env)
  (unless (and (listp fun)
    ...
    (error-of-type 'source-program-error
      (DEUTSCH "SETF von FUNCALL ist nur fåÕr Funktionen der Form #'symbol definiert."
       ENGLISH "SETF FUNCALL is only defined for functions of the form #'symbol."
       FRANCAIS "Un SETF de FUNCALL n'est då∏fini que pour les fonctions de la forme #'symbole.")
  ) )
  (get-setf-expansion (cons fun args) env)
)
;;;----------------------------------------------------------------------------
