  ...
  global object read_rational(base,sign,string,index1,index3,index2)
    ...
    var uintL index2;
    {
      pushSTACK(string); # string retten
      {
        ...
        if (eq(x,Fixnum_0)) # Division durch 0 abfangen
          divide_0();
  ...
  global object read_float(base,sign,string,index1,index4,index2,index3)
    ...
    var uintL index3;
    {
      ...
      if (exp_len > 0) {
        ...
        if (!(exp_sign==0))
          exponent = I_minus_I(exponent); # incl. Vorzeichen
      } else {
        # kein Exponent da
...
# (SETF (EXT:LONG-FLOAT-DIGITS) digits) = (SYS::%SET-LONG-FLOAT-DIGITS digits)
  {
    var object arg = STACK_0;
    if (!posfixnump(arg)) # nicht notwendig Fixnum!??
      fehler_digits(arg);
