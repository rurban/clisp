    ...
    (do ((l intervals (cddr l)))
      ...
      (let* ((lo (car l))
        ...
        (flet ((mapto (value)
          ...
          (cond (lo-incl
                 (multiple-value-bind (x force-excl) (mapto lo-incl)
        ...
        (flet ((mapto (value)
                 (case kind
                   ...
                   (t (values (coerce value kind) nil)))))
          (cond (hi-incl
                 (multiple-value-bind (x force-excl) (mapto hi-incl)
...
(defun intervals-remove-one (intervals kind obj)
  (let ((new-intervals '()))
    (flet ((add-new-interval (lo-incl lo-excl hi-incl hi-excl)
             (when (eq kind 'INTEGER)
               (when lo-excl (setq lo-incl (+ lo-excl 1) lo-excl nil))
               (when hi-excl (setq hi-incl (- hi-excl 1) hi-excl nil)))
             (when (interval-lohi-<= lo-incl lo-excl hi-incl hi-excl)
               (push (if lo-incl lo-incl (if lo-excl `(,lo-excl) '*))
                     new-intervals)
               (push (if hi-incl hi-incl (if hi-excl `(,hi-excl) '*))
                     new-intervals))))
      (do ((l intervals (cddr l)))
          ((endp l))
        (let* ((lo (car l))
         ...
         (case head
           ...
           ((REAL RATIONAL INTEGER FLOAT
            ...
            (let ((lo (if (cdr type) (second type) '*))
              ...
              (flet ((valid-interval-designator-p (bound)
                ...
                (unless (and (valid-interval-designator-p lo)
                             (valid-interval-designator-p hi))
                  (typespec-error 'subtypep type))
...
(defun subtypep-logic (types1 types2)
  (macrolet ((yes () '(return-from subtypep-logic (values t t)))
    ...
    (let ((type1parts '())
          (type2parts '())
          (type2eqlparts '()))
      (labels (; Add a canonicalized type to the left-hand side: type1parts.
               (flatten-AND (type)
                 (if (consp type)
                   (if (eq (first type) 'AND)
                     (mapc #'(lambda (x)
                               (flatten-AND (canonicalize-type x)))
                           (rest type))
                     (if (eq (first type) 'NOT)
                       (flatten-OR (canonicalize-type (second type)))
                       (push type type1parts)))
                   (push type type1parts)))
               ...
               (flatten-OR (type)
                 (if (consp type)
                   (if (eq (first type) 'OR)
                     (mapc #'(lambda (x)
                               (flatten-OR (canonicalize-type x)))
                           (rest type))
                     (if (eq (first type) 'NOT)
                       (flatten-AND (canonicalize-type (second type)))
                       (if (eq (first type) 'MEMBER)
                         (setq type2eqlparts
                           (revappend (rest type) type2eqlparts))
                         (if (eq (first type) 'EQL)
                           (push (second type) type2eqlparts)
                           (push type type2parts)))))
                   (push type type2parts))))
        (mapc #'flatten-AND types1)
        (mapc #'flatten-OR types2))
