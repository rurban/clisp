...
/* Each of these macros verifies that its argument R is nonzero.  To
   ...
   * For the verify macro, the struct verify_type__ will need to
     ...
     attach the current line number to the entity name:
       ...
       #define _gl_GL_CONCAT0(x, y) x##y
       #define _gl_GL_CONCAT(x, y) _gl_GL_CONCAT0 (x, y)
       extern struct {...} * _gl_GL_CONCAT (dummy, __LINE__);
...
/* _gl_GL_COUNTER is an integer, preferably one that changes each time we
   use it.  Use __COUNTER__ if it works, falling back on __LINE__
   otherwise.  __LINE__ isn't perfect, but it's better than a
   constant.  */
# if defined __COUNTER__ && __COUNTER__ != __COUNTER__
#  define _gl_GL_COUNTER __COUNTER__
# else
#  define _gl_GL_COUNTER __LINE__
# endif

/* Generate a symbol with the given prefix, making it unique if
   possible.  */
# define _gl_GL_GENSYM(prefix) _gl_GL_CONCAT (prefix, _gl_GL_COUNTER)
...
# else
#  define verify(R) \
    extern int (* _gl_GL_GENSYM (verify_function) (void)) [verify_true (R)]
# endif
