/* Basic Single-Float functions */

/* Unpack a Single-Float:
 FF_decode(obj, zero_statement, sign=,exp=,mant=);
 unpacks a Single-Float obj.
 When obj=0.0, zero_statement is executed.
 Else: signean sign = Sign (0 = +, -1 = -),
       sintWL exp   = Exponent (signed),
       uintL mant   = Mantissa (>= 2^FF_mant_len, < 2^(FF_mant_len+1)) */
...
#define ffloat_decode(val, zero_statement, sign_assignment,exp_assignment,mant_assignment)  \
    {                                                                        \
      ...
      if (uexp==0) {                                                         \
        zero_statement /* e=0 -> 0.0 */                                      \
      } else {                                                               \
        exp_assignment (sintWL)((uintWL)uexp - FF_exp_mid); /* Exponent */   \
        unused (sign_assignment sign_of_sint32((sint32)(val))); /* Sign */   \
...
/* Pack a Single-Float:
 ...
 > sintWL exp: Exponent
 > uintL mant: Mantissa, should be  >= 2^FF_mant_len and < 2^(FF_mant_len+1).
 < object result: a Single-Float
 Exponent ist tested for over/underflow.
 can trigger GC */
...
/* Unpack a Float: */
  ...
  #else /* defined(WIDE) -> one of the two 32-bit words */
    #ifdef GNU
...
/* Check and pack one of the float-functions returned IEEE float.
 Classification:
...
#define float_to_FF(expr,result_assignment,maybe_overflow,maybe_subnormal,maybe_underflow,maybe_divide_0,maybe_nan)  \
    {                                                                    \
      ...
      if ((_erg.eksplicit &                                              \
           ((uint32)bit(FF_exp_len+FF_mant_len)-bit(FF_mant_len))) == 0) { /* e=0 ? */ \
        if ((maybe_underflow                                             \
             || (maybe_subnormal && !((_erg.eksplicit << 1) == 0))       \
            )                                                            \
            && underflow_allowed()                                       \
           ) {                                                           \
          error_underflow(); /* subnormal or smaller -> Underflow */     \
        } else {                                                         \
      ...
      if ((maybe_overflow || maybe_divide_0)                             \
          && (((~_erg.eksplicit) &                                       \
               ((uint32)bit(FF_exp_len+FF_mant_len)-bit(FF_mant_len))) == 0) /* e=255 ? */ \
             ) {                                                         \
        if (maybe_nan && !((_erg.eksplicit << (32-FF_mant_len)) == 0)) { \
          /* NaN, and singularity -> "Division by 0" */                  \
          divide_0();                                                    \
        } else {                                                         \
          /* Infinity */                                                 \
          if (!maybe_overflow || maybe_divide_0)                         \
...
/* Returns for a Single-Float x : (ftruncate x), a FF.
 FF_ftruncate_FF(x)
 x wird zur 0 hin zur nächsten ganzen Zahl gerundet.
 can trigger GC */
local maygc object FF_ftruncate_FF (object x);
/* Method:
   x = 0.0 or e<=0 -> return 0.0
   1<=e<=23 -> set last (24th) bit of mantissa to 0,
...
/* Returns for a Single-Float x : (futruncate x), a FF.
 FF_futruncate_FF(x)
...
/* Returns for a Single-Float x : (fround x), ein FF.
 FF_fround_FF(x)
...
/* Returns for a Single-Float x : (- x), ein FF.
 FF_minus_FF(x)
 can trigger GC */
...
/* Method ([Knuth, II, Seminumerical Algorithms, section 4.2.1., p.200]):
 x1=0.0 -> Ergebnis x2.
 x2=0.0 -> Ergebnis x1.
...
/* Returns for a Single-Float x>=0 : (sqrt x), ein FF.
   FF_sqrt_FF(x)
   can trigger GC */
