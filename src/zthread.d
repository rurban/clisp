...
/* UP: fills timespec with millis milliseconds form "now"
   ...
   > millis: milliseconds */
static inline void get_abs_timeout(struct timespec *r, uintL millis) {
  var struct timeval tv;
...
/* UP: Implements waiting on xlock_t in "polling" mode - so async POSIX signals
   ...
   > poll_interval: the granularity of polling (in order to be able to process
     async interrupts)
  ...
  if (pthread_equal(l->_owner,pthread_self())) {
    ...
    if (lock_real) {
      ...
      while (r = pthread_mutex_trylock(&l->_mr)) {
        var uintC wait = MIN(poll_interval, timeout);
        if (!wait) break;
        var struct timespec ww;
        get_abs_timeout(&ww,wait);
        /* following is cancellation point - but nobody is going to cancel us*/
        r = pthread_cond_timedwait(&l->_c,&l->_m,&ww);
        if (thr && thr->_pending_interrupt_count) {
          /* interrupt may exit non-locally - so unlock mutex */
          pthread_mutex_unlock(&l->_m);
          handle_pending_interrupts_gc();
          pthread_mutex_lock(&l->_m); /* lock again */
        }
        if (timeout != XLOCK_WAIT_INFINITE) timeout -= wait;
        if (r != 0 && r != ETIMEDOUT) break;
      }
      if (!l->_owner && r == 0) {
        l->_owner = pthread_self();
        l->_count=1;
      } else if (r == 0) r = ETIMEDOUT;
    } else {
                           ...
                           uintL poll_interval)
{
  ...
  while (!r) {
    ...
    if (thr && thr->_pending_interrupt_count) {
      xlock_lock_helper(m,0,0,false); /* claim ownership */
      handle_pending_interrupts_gc();
      xlock_unlock_helper(m,false); /* give up ownership */
    }
    if (timeout != XLOCK_WAIT_INFINITE) timeout -= wait;
    if (r != 0 && r != ETIMEDOUT) break;
  }
  /* mark again the mutex as ours */
  xlock_lock_helper(m,0,0,false);
  return r;
}
