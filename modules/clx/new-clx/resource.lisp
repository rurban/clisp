...
(defmethod print-object ((database resource-database) (stream stream))
...
(defun root-resources (screen &key database key test test-not)
  ...
  (let* ((screen (if (typep screen 'display)
...
(defun default-resources-pathname ()
  (make-pathname :name ".Xdefaults" :defaults (user-homedir-pathname)))
(defun resources-pathname ()
  (or (let ((string (ext:getenv "XENVIRONMENT")))
	(and string (pathname string)))
     (make-pathname
      :name (ext:string-concat
             ".Xdefaults-" (let ((s (machine-instance))) ; hostname
