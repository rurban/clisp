...
(defun radians->int16 (value)
  ...
  (declare (values int16))
  #.(declare-buffun)
  (the int16 (values (round (the angle value) #.(/ (float pi 0s0) 180s0 64s0)))))
...
(defun int16->radians (value)
  ...
  (declare (values short-float))
  #.(declare-buffun)
  (the short-float (* (the int16 value) #.(/ (float pi 1s0) 180s0 64s0))))
...
#-(or Genera explorer excl lcl3.0 Minima CMU CLISP akcl)
(defun buffer-read-default (display vector start end timeout)
  (declare (type display display)
	   (type buffer-bytes vector)
	   (type array-index start end)
...
#-(or Genera explorer excl lcl3.0 Minima CMU CLISP)
(defun buffer-write-default (vector display start end)
  ;; The default buffer write function for use with common-lisp streams
  (declare (type buffer-bytes vector)
	   (type display display)
...
#+(and clx-overlapping-arrays (not (or lispm lucid excl cmu)))
(defun buffer-replace (buf1 buf2 start1 end1 &optional (start2 0))
  (declare (type vector buf1 buf2)
	   (type array-index start1 end1 start2))
...
#+(or clx-ansi-common-lisp excl lcl3.0 (and kcl clos-conditions) (and CLISP have-clcs))
(defun x-error (condition &rest keyargs)
  #-kcl (declare (dynamic-extent keyargs))
...
(defun x-error (condition &rest keyargs)
  (let ((condx (apply #'make-condition condition keyargs)))
    (when (and (eq condition 'closed-display)
	       (fboundp 'ext::disable-clx-event-handling))
      (let ((disp (closed-display-display condx)))
	(warn "Disabled event handling on ~S." disp)
	(ext::disable-clx-event-handling disp)))
    (error condx)))

#-(or lispm ansi-common-lisp excl lcl3.0 CMU (and kcl clos-conditions) (and CLISP have-clcs))
(defun x-error (condition &rest keyargs)
  (error "X-Error: ~a"
...
#-(or clx-ansi-common-lisp Genera CMU (and CLISP have-with-standard-io-syntax))
...
#+(or (not (or cmu clx-ansi-common-lisp)) lispm allegro)
...
#+(and (or cmu clx-ansi-common-lisp) (not lispm) (not allegro))
(defun default-keysym-translate (display state object)
  (declare (type display display)
	   (type card16 state)
	   (type t object)
