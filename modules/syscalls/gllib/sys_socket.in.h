...
# else
   _sc_gl_GL_WARN_ON_USE (gethostname,
...
/* Need to cast, because on NonStop Kernel, the third parameter is
                                                     size_t addrlen.  */
_sc_gl_GL_CXXALIAS_SYS_CAST (connect, int,
                       (int fd,
...
/* Need to cast, because on NonStop Kernel, the third parameter is
                                                     size_t addrlen.  */
_sc_gl_GL_CXXALIAS_SYS_CAST (bind, int,
                       (int fd,
                        const struct sockaddr *addr, socklen_t addrlen));
# endif
...
/* Need to cast, because on NonStop Kernel, the sixth parameter is
                                                   size_t tolen.  */
_sc_gl_GL_CXXALIAS_SYS_CAST (sendto, ssize_t,
                       (int fd, const void *buf, size_t len, int flags,
                        const struct sockaddr *to, socklen_t tolen));
# endif
...
/* Need to cast, because on NonStop Kernel, the fifth parameter is
                                             size_t optlen.  */
_sc_gl_GL_CXXALIAS_SYS_CAST (setsockopt, int,
