...
#if defined(HAVE_ERRNO_H) && defined(HAVE_STRERROR)
DEFCHECKER(check_errno, ECANCELED ENOTSUP EPERM ENOENT ESRCH EINTR EIO ENXIO \
           ...
           ENOTTY ETXTBSY EFBIG ENOSPC ESPIPE EROFS EMLINK EPIPE EDOM ERANGE \
           EDEADLK ENAMETOOLONG ENOLCK ENOSYS ENOTEMPTY ELOOP EWOULDBLOCK \
           ENOMSG EIDRM ECHRNG EL2NSYNC EL3HLT EL3RST ELNRNG EUNATCH ENOCSI \
           EL2HLT EBADE EBADR EXFULL ENOANO EBADRQC EBADSLT EDEADLOCK EBFONT \
           ENOSTR ENODATA ETIME ENOSR ENONET ENOPKG EREMOTE ENOLINK EADV \
...
DEFUN(OS::STRERROR, &optional error_code) {
  int e = missingp(STACK_0) ? errno : check_errno(STACK_0);
  char *ret;
  begin_system_call(); ret = strerror(e); end_system_call();
  VALUES1(safe_to_string(ret));
  skipSTACK(1);
}
#endif  /* HAVE_ERRNO_H & HAVE_STRERROR */
