...
(progn (defpackage "FTEST" (:use "FFI" "COMMON-LISP")) T) T

(progn (in-package "FTEST") T) T
...
(typep (setq v0 (ffi::lookup-foreign-variable "ffi_user_pointer"
...
;; c-array uint8/16/32 was broken with in CLISP <= 2.28, while sint8/16 worked!
(c-self #(128 255 0 127))
#(128 255 0 127)
...
(def-call-out foreign-as-string
  (:name "ffi_identity")
  (:arguments (obj c-pointer))
  (:return-type c-string) (:language :stdc))
FOREIGN-AS-STRING

(with-foreign-string (fv e b "ABC" #+UNICODE :encoding #+UNICODE charset:ascii :null-terminated-p nil)
  (list e b))
(3 3)

(with-foreign-string (fv e b "ABC" #+UNICODE :encoding #+UNICODE charset:ascii :null-terminated-p t)
  (list e b))
(4 4)
...
(foreign-value (ffi::%cast fm (ffi::parse-c-type '(c-ptr (c-array-max character 20)))))
"abc"
...
; broken in clisp <= 2.30
(with-foreign-object (x '(c-struct list (a (c-array-ptr ulong)) (b (c-array-ptr ulong)) (c (c-array-ptr ulong)) (d (c-ptr (c-array ulong 2)))) '(#(123456789) #(987654321) #(543235263) #(936272894 1333222444))) (foreign-value x))
...
; prevent from shooting in the foot
(setf (validp (unsigned-foreign-address 4)) nil)
ERROR

#+UNICODE
(progn (setq custom:*foreign-encoding* orig-encoding) T)
#+UNICODE T
