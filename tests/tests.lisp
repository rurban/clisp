...
(defmacro check-os-error (form acceptable)
  (let ((errvar (gensym "ERROR-")))
    `(handler-case ,form
       (os-error (,errvar)
         (princ-error ,errvar)
         (case (os-error-code ,errvar)
           (,acceptable T)
           (t (os-error-code ,errvar)))))))
...
(defun symbol-cleanup (s)
  ...
  #+CLISP (let ((tldl (sys::top-level-declarations)))
            (when tldl
              (setf (cdr tldl) (delete s (cdr tldl) :test #'eq))))
